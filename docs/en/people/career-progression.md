---
description: Ultralytics career progression framework overview with IC and Management tracks, promotion process, and track switching guidance.
keywords: Ultralytics careers, job levels, career progression, promotion criteria, IC levels, management levels, career ladder, YOLO11
---

# Career Progression Framework

Ultralytics' career framework provides clear expectations for each level and guides professional development. We maintain two career tracks—Individual Contributor (IC) and Management—allowing team members to advance based on their strengths and interests.

## Career Tracks Overview

### Individual Contributor (IC) Track

Focused on technical excellence, problem-solving, and individual impact. IC roles advance through deepening expertise and expanding scope of influence without managing people.

**Progression**: IC02 (Associate/Junior) → IC03 (Professional) → IC04 (Senior) → IC05 (Staff) → IC06 (Senior Staff) → IC07 (Principal)

**See detailed expectations**: [IC Levels (IC02-IC07)](ic-levels.md)

### Management Track

Focused on achieving results through others, building teams, and organizational leadership. Management roles balance people leadership with technical or domain expertise.

**Progression**: M04 (Lead Manager) → M05 (Manager) → M06 (Senior Manager) → M07 (Head of) → M08 (Director/VP)

**See detailed expectations**: [Management Levels (M04-M08)](management-levels.md)

!!! info "Equal Value"
    Both IC and Management tracks are valued equally and offer equivalent compensation and growth potential at similar scope levels. Choose the track that best aligns with your strengths and career goals.

---

## Track Comparison

| Aspect | Individual Contributor | Management |
|--------|------------------------|------------|
| **Impact Through** | Technical expertise and individual work | Building and leading teams |
| **Key Skills** | Deep technical knowledge, problem-solving, innovation | People development, strategic planning, organizational leadership |
| **Influence** | Technical decisions, architecture, best practices | Team performance, culture, organizational direction |
| **Scope Growth** | Project → Team → Multi-team → Organization | Team → Department → Function → Division |
| **Focus Evolution** | 100% Ops → 60% Ops / 40% Strategic | 80% Ops → 40% Ops / 60% Strategic |

---

## Switching Career Tracks

Team members can move between IC and Management tracks based on interests, strengths, and business needs. This flexibility allows everyone to find their optimal contribution path.

### IC to Management

**When to Consider**:

- Passionate about developing people and building teams
- Enjoy navigating organizational dynamics and stakeholder management
- Want to influence through others' work rather than individual contributions
- Strong leadership and communication skills

**Typical Path**: IC04/IC05 → M04 (often with mentor/shadow period)

**Requirements**:

- Demonstrated technical excellence at current level
- Leadership examples: mentoring, project leadership, cross-team collaboration
- Manager endorsement and leadership interest discussion
- 12+ months in current role
- Business need for management position

### Management to IC

**When to Consider**:

- Miss hands-on technical work and individual contribution
- Prefer influence through expertise rather than organizational authority
- Want deeper specialization in technical domain
- Enjoy solving complex problems directly

**Typical Path**: M04/M05 → IC05/IC06 (depending on tenure and expertise)

**Requirements**:

- Maintain technical currency alongside management responsibilities
- Discussed with manager as part of career planning
- Business need and appropriate IC role available
- Smooth team transition plan

!!! note "Track Switching Support"
    Career track changes are normal and supported. Discuss with your manager during quarterly reviews. We value both paths equally and compensate based on scope and impact, not track choice.

---

## Promotion Process

### Promotion Criteria

Promotions require consistent demonstration of next-level performance across **all dimensions**:

1. **Performance**: Meeting/exceeding current level expectations for 6+ months
2. **Next-Level Work**: Consistently operating at next level for 6-12 months
3. **Scope**: Taking on expanded scope and complexity appropriate to next level
4. **Impact**: Demonstrated impact on team, function, or company
5. **Readiness**: Manager and skip-level agreement on promotion readiness

### Promotion Timeline

- **Quarterly Reviews**: Discuss progress toward next level with manager
- **Self-Nomination**: Indicate interest in promotion consideration
- **Manager Assessment**: Manager evaluates against level criteria
- **Calibration**: Management team ensures consistency across organization
- **Decision & Communication**: Typically within 2-4 weeks of nomination
- **Effective Date**: Usually next quarter start, with compensation adjustment

### Building a Promotion Case

Strong promotion cases include:

- **Performance Examples**: 3-5 specific examples demonstrating next-level impact
- **Scope Evidence**: Projects, decisions, and influence at next level
- **Peer Feedback**: Input from collaborators and stakeholders
- **Growth Trajectory**: Pattern of increasing impact over time
- **Business Impact**: Quantified results where possible

### Self-Advocacy Tips

!!! tip "Demonstrating Readiness"
    - **Document Wins**: Keep running list of achievements and impact
    - **Seek Feedback**: Regularly ask manager about readiness and gaps
    - **Fill Gaps**: Proactively take on next-level work and responsibilities
    - **Build Visibility**: Share work in demos, docs, and cross-team forums
    - **Be Patient**: Promotions reward sustained performance, not single achievements

---

## Compensation & Levels

### Compensation Philosophy

Compensation reflects scope, impact, and market rates rather than tenure alone. Both IC and Management tracks offer equivalent compensation potential at similar scope levels.

**Compensation Components**:

- **Base Salary**: Market-competitive based on level, location, and role
- **Equity**: Meaningful ownership through stock options
- **Bonuses**: Performance-based for exceptional contributions
- **Benefits**: Comprehensive benefits package

### Leveling Factors

Compensation bands consider:

- **Geographic Location**: Adjusted for market rates and cost of living
- **Role Specialization**: Some roles (e.g., ML/AI, Security) command higher market rates
- **Performance**: High performers may be positioned higher in band
- **Experience**: Prior experience may influence initial placement

!!! info "Compensation Reviews"
    Compensation is reviewed quarterly during performance reviews. Adjustments consider individual performance, promotion eligibility, market changes, and company performance. Discuss compensation questions with your manager.

---

## Growth & Development

### Development Resources

**Learning Budget**: Annual allocation for courses, books, conferences, certifications (contact HR for current amounts)

**Internal Growth**:

- Quarterly 1:1s with manager for career planning
- Mentorship programs pairing with senior team members
- Internal tech talks, workshops, and knowledge sharing
- 10% time for exploring new technologies (with manager approval)

**External Growth**:

- Conference attendance for relevant industry events
- Online courses (Coursera, Udemy, Pluralsight, etc.)
- Certifications (AWS, GCP, Azure, product management, etc.)
- Books and technical publications

### Career Planning Process

**Quarterly Review Discussion Points**:

1. Progress toward current level mastery
2. Readiness for next level
3. Specific skills or experiences needed
4. Opportunities to demonstrate next-level work
5. Career interests (IC vs. Management, technical areas)
6. Learning and development goals

**Creating Your Growth Plan**:

1. **Self-assess**: Review [IC Levels](ic-levels.md) or [Management Levels](management-levels.md) criteria
2. **Identify Goals**: Choose 2-3 skills or experiences for next quarter
3. **Find Opportunities**: Work with manager to get exposure to next-level work
4. **Get Feedback**: Regularly check progress with manager and peers
5. **Document Impact**: Track achievements for promotion discussions

---

## Typical Promotion Timelines

### IC Track

- **IC02 → IC03**: 12-18 months demonstrating independent feature delivery
- **IC03 → IC04**: 18-24 months demonstrating consistent senior-level impact
- **IC04 → IC05**: 24-36 months demonstrating cross-team technical leadership
- **IC05 → IC06**: 36+ months demonstrating department-level strategic impact
- **IC06 → IC07**: 36+ months demonstrating organization-wide strategic impact

### Management Track

- **M04 → M05**: 18-24 months demonstrating multi-team coordination excellence
- **M05 → M06**: 24-36 months demonstrating functional leadership and strategic impact
- **M06 → M07**: 36+ months demonstrating executive-level influence
- **M07 → M08**: 48+ months demonstrating senior executive leadership

!!! warning "Sustained Performance Required"
    Timelines reflect typical progression for sustained next-level performance. Exceptional contributions may accelerate timelines, but promotions always require demonstrating readiness across **all dimensions** of the next level.

---

## Using This Framework

### For Individual Contributors

1. **Review Your Level**: Read [IC Levels](ic-levels.md) thoroughly for your current level
2. **Self-Assess**: Honestly evaluate against each dimension of expectations
3. **Identify Gaps**: Find 1-2 areas to address this quarter
4. **Study Next Level**: Understand the path forward clearly
5. **Discuss with Manager**: Align on assessment and create development plan

### For Managers

1. **Review Your Level**: Read [Management Levels](management-levels.md) thoroughly for your current level
2. **Self-Assess**: Evaluate people development, strategic thinking, and organizational impact
3. **Identify Gaps**: Find areas to strengthen management capabilities
4. **Study Next Level**: Understand increasing scope and strategic expectations
5. **Discuss with Skip-Level**: Align on leadership development priorities

### For Hiring Managers

- Use level criteria during job leveling and offer determination
- Share expectations with candidates to set clear role expectations
- Reference during onboarding to align on goals and success metrics
- Apply consistently in performance reviews across organization
- Guide promotion discussions with concrete, evidence-based criteria

---

## Questions & Support

### Career Development Questions

- **Manager**: Primary resource for career discussions and promotion readiness
- **HR Team**: Questions about levels, compensation, or promotion process
- **Senior ICs/Managers**: Seek mentorship from those in roles you aspire to
- **Skip-Level**: Discuss career with manager's manager for additional perspective

### Additional Resources

- **[IC Levels (IC02-IC07)](ic-levels.md)** - Detailed Individual Contributor expectations
- **[Management Levels (M04-M08)](management-levels.md)** - Detailed Management expectations
- **[People & Benefits](index.md)** - Comprehensive people policies and resources
- **[Onboarding](onboarding.md)** - Level expectations for new team members

---

*Your career at Ultralytics is a journey, not a destination. We're committed to supporting your growth whether you aspire to deep technical expertise or organizational leadership. Let's build amazing things together! 🚀*
